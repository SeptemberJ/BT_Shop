...
Page({
  ...
  onShow:function(){
    wx.request({
      ...
      success: (res)=>{
        ...
        if (res.data.arr[1].length>0){
          res.data.arr[1].map(function(item,index){
            var format = new Date(res.data.arr[1][index].comment_time.time)
            temp[1][index].comment_time.time = utils.secondToFormat(format)
          })
        }
        console.log(temp)
        this.setData({
  ...
  previewImage: function (e) {
    ...
    wx.previewImage({
      current: this.data.ware[0][0].fimgs1[currentImgIndex],
      urls: this.data.ware[0][0].fimgs1,
  ...
  confirm: function(){
    if (this.data.cartOrPay==1){
      ...
      orderInfo.total = Number(buyGoodInfo.goods_price) * buyGoodInfo.goods_num
      wx.setStorage({
        key: "orderInfo",
        data: orderInfo
      })
